...
public class CsvParserBenchmarks
{
    ...
    public async Task<int> ParseLoansAsync() =>
        (await _parser.ParseAsync<Loan>(Path.Combine(_csvDirectory, "loans.csv"))).Value.Count;

    [Benchmark]
    public async Task<int> ParsePortfoliosAsync() =>
        (await _parser.ParseAsync<Portfolio>(Path.Combine(_csvDirectory, "portfolios.csv"))).Value.Count;

    [Benchmark]
    public async Task<int> ParseRatingsAsync() =>
        (await _parser.ParseAsync<Rating>(Path.Combine(_csvDirectory, "ratings.csv"))).Value.Count;
}
